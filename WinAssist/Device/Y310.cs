...
namespace WinAssist.Device
{
    public  class Y310 : CenterCom
    {
        ...
        public Y310(string strVersion)
        {
          ...
          //不同型号不同锁控
        public override int LockID(List<int> temp)
        {
            //称重就是开锁
            foreach (int tempint in temp)
            {
                OpenLock(tempint.ToString(), "1");
                Thread.Sleep(100);
            }
            return 0;
        }
        ...
        public override void AnayzeData(byte[] bytes, List<byte> newbytes, byte Modeltype)
        {
            ...
            switch (Modeltype)
            {
                case 0x07://精准药盒模块  
                    ...
                    switch (receiveBytes[3])
                    {
                        ...
                        case 0x02: //统计数量
                            ...
                            arr = Convert.ToString(newbytes[7], 2).PadLeft(8, '0') + Convert.ToString(newbytes[8], 2).PadLeft(8, '0') + Convert.ToString(newbytes[9], 2).PadLeft(8, '0');
                            listobj.Add(arr);
                            int leng3 = arr.Length - arr.Replace("1", "").Length;
                            listobj.Add(leng3);
                            SetReceiveType(listobj, CmdType.Cmd_TJCount);
                            break;
                ...
                case 0x01://锁模块 一般就是称重，或回收箱
                    ...
                    {
                        case 0x00: //关锁 状态
                            /// LogManager.WriteLogSave("GS", DevID.ToString()+">>"+StrCom.byteToHexStr(receiveBytes, receiveBytes.Length));
                            //关锁查询数量
                            Thread.Sleep(50);
                            QMessage.Enqueue(DevID.ToString());
                            //移动小车按编号
                            //SendReadCount(DevID.ToString(), 0);
                            break;
                        case 0x01: // 开锁 状态
                            SetReceiveType(DevID.ToString(), CmdType.Cmd_KS);
                            //LogManager.WriteLogSave("kS", DevID.ToString() + ">>" + StrCom.byteToHexStr(receiveBytes, receiveBytes.Length));
                    ...
                    switch (receiveBytes[3])
                    {
                        ...
                        case 0x05: //查询数量
                            ...
                            if (receiveBytes[2] == 0x05)
                            {
                                ...
                                //LogManager.WriteLogSave("ReadCount", DevID.ToString() + ">>盘：" + receiveBytes[4].ToString() + " 数量：" + weightNums1.ToString());
